...
function pick_flower_from_field()
	...
	if flower and flower:is_adult() then
		...
		if not harvested_flower_inventory[display_str] then
			harvested_flower_inventory[display_str] = 0
		end
		harvested_flower_inventory[display_str] += 1
		flower:set_growth_state(2,growth_stage_timings[3])
		sync_flower_sprite(fc_x, fc_y)
	end
end
...
function draw_toolbar()
	draw_filled_rrect(
		...
		0,
		4, 7)
	if selected_tool_spr then
		spr(selected_tool_spr,3,116)
	end
...
function flower_class:create(chr1,chr2)
	...
	--with 1 week on growth timer
	f:set_growth_state(0,1)
	return f
end
...
growth_stage_timings = split"1,1,2,3,2"
...
function time_passes()
	...
	for_all_flowers(function(flower)
		...
		if growth_time == 1 then
			flower:set_growth_state(
				growth_stage == 3 and 2 or growth_stage + 1,
				growth_stage_timings[growth_stage+2]
			)
		else 
	...
	--expire harvested flowers
	local total_expired_flowers = 0
	for display_str,flower_count in pairs(harvested_flower_inventory) do
		total_expired_flowers += flower_count
	end
	harvested_flower_inventory = {}
	if total_expired_flowers > 0 then
		open_toast_menu(
			total_expired_flowers.." flowers expired",
			function() end)
	end
...
function open_toast_menu(text,on_close)
	open_menu(on_close)
	menu_title = text
	options = {split"15,continue,1"}
	menu_min_x,menu_min_y = 20,80
	menu_w,menu_h = 88,24
end
...
function save_inventory(addr)
	...
	for display_str,flower_count in pairs(harvested_flower_inventory) do
		...
		addr = save_num(addr, flower_count)
	end
...
function load_inventory(addr)
	...
	for i=1,inventory_size do
		...
		flower_count, addr = load_num(addr)
		harvested_flower_inventory[display_string] = flower_count
	end
	return addr
end
		...
		if ic_y == ic_max_y then
			if btnp(🅾️) then
				for i = 1,ic_max_y - 1 do
					local display_str,selected,flower_count = unpack(i_inventory[i])
					if selected == flower_count then
						harvested_flower_inventory[display_str] = nil
					else
						harvested_flower_inventory[display_str] -= selected
					end
				end
			...
			local _,selected,flower_count = unpack(inventory_row)
			if btnp(⬅️) and selected > 0 then
				inventory_row[2] -= 1
				i_total_selected -= 1
			elseif btnp(➡️) and selected < flower_count and (not i_limit or i_total_selected < i_limit) then
				inventory_row[2] += 1
				i_total_selected += 1
			end
		end
...
function draw_inventory_screen_info()
	...
	for i = 1,#i_inventory do
		...
		if i_select_mode and i == ic_max_y then
			...
			display_str,selected,flower_count = unpack(i_inventory[i])
...
__gfx__

